...
fun scrollerItem(
    ...
    onBundleSelect {
        handleBundleScroll(window.get(), line, maxLine, bundleSlot, lines)
    }
...
private fun handleBundleScroll(
    ...
    val targetLine = when (determineScrollDirection(previousSlot, bundleSlot)) {
        1 -> line - lines
        -1 -> line + lines
        else -> line
    }.coerceIn(0..maxLine)
 ...
 */
internal fun determineScrollDirection(from: Int, to: Int): Int = when (to) {
    -1 -> 0 // release
